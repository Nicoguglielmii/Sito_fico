...
export function LogoAnimation({ className = "", onDotClick, dotLabels }: Props) {
  ...
  useEffect(() => {
    ...
    const paths = Array.from(svg.querySelectorAll<SVGPathElement>("[data-arc]"))
      .sort((a, b) => +a.dataset.arc! - +b.dataset.arc!);
    const dots = Array.from(svg.querySelectorAll<SVGCircleElement>("[data-dot]"))
      .sort((a, b) => +a.dataset.dot! - +b.dataset.dot!);
    ...
    function resetAll() {
      ...
      dots.forEach((d) => {
        ...
        d.setAttribute("transform",
          `translate(${cx} ${cy}) scale(0) translate(${-cx} ${-cy})`);
      });
    }
    resetAll();
    ...
    const timing: [number, number][] = [
      ...
      [0.21, 0.46], // arc3 → dot3
      [0.29, 0.46], // arc4 → dot4
      [0.40, 0.42], // arc5 → dot5 (Permitting) — appare a t=0.82
    ];
    ...
    <>
      ...
      <div className={`relative w-full ${className}`}>
        ...
        >
          ...
          <path data-arc="4" d="M 290 490 C 330 545, 680 530, 820 415"
            stroke="url(#lg6)" strokeWidth="32" strokeLinecap="round" fill="none" />
          {/* arc5 – destra-alto */}
          <path data-arc="5" d="M 290 490 C 430 420, 720 250, 860 180"
            stroke="url(#lg6)" strokeWidth="32" strokeLinecap="round" fill="none" />
          {/* arc2 – sinistra-basso */}
          <path data-arc="2" d="M 290 490 C 250 530, 160 535, 90 500"
            stroke="url(#lg3)" strokeWidth="32" strokeLinecap="round" fill="none" />
          {/* arc1 – sinistra-medio */}
          <path data-arc="1" d="M 290 490 C 200 490, 100 420, 60 305"
            stroke="url(#lg2)" strokeWidth="32" strokeLinecap="round" fill="none" />
          {/* arc3 – centro-alto */}
          <path data-arc="3" d="M 290 490 C 350 380, 480 210, 530 100"
            stroke="url(#lg4)" strokeWidth="32" strokeLinecap="round" fill="none" />
          {/* arc0 – sinistra-alto (primo piano) */}
          <path data-arc="0" d="M 290 490 C 240 390, 170 250, 120 120"
            stroke="url(#lg1)" strokeWidth="32" strokeLinecap="round" fill="none" />
        ...
        {labelsVisible && dotLabels?.map((label, i) => (
          <div
            key={`label-${i}`}
            className="fco-label"
            style={{
